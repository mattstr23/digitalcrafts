...
async function fetchPokemonData(){
    ...
    for (const singlePokeData of jsonPokemonData.results) {
        const pokeCard = document.createElement("div");
        pokeCard.className = "pokeCard";
        const pokeName = document.createElement("h4");
        pokeName.innerHTML = singlePokeData.name;
        pokeCard.append(pokeName);
        cardContainer.append(pokeCard);
    }
    for (const pokemon of data) {
      const pokeImageAdd = document.querySelectorAll(".pokeCard");
      for (let index = 0; index < pokeImageAdd.length; index++) {
        const element = pokeImageAdd[index];
        const pokeImg = document.createElement("img");
        pokeImg.scr = pokemon.pokemon[index].sprites.front;
        element.append(pokeImg);
      }
    }
